...
describe("Node HTTPS unit test", () => {
  const fakeApiURL: string = "http://someapi/api";
  const fakeAccessKey: string = "FAKEKEY";
  let axiosRequester: IHttpRequest;
  let accessGeolocationAPI: IHttpRequest;

  beforeAll(() => {
    axiosRequester = new AxiosHttpRequest(fakeApiURL);
    accessGeolocationAPI = new AccessGeolocationAPI(axiosRequester, fakeAccessKey);
  })
  ...
  it("Should make a get request for a json api with first parameter as \"access_key\"", async () => {
    ...
    await expect(accessGeolocationAPI.get(fakePath, queryParams)).resolves.toEqual(fakeResponse);
  })
})