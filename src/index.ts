...
import { IConsumer, IEventHandler, topic } from "./infra/stream/@core/consumer.interface";
...
export const main = async (cache: ICache, producer: IProducer, consumer: IConsumer, apiGeolocationRequester: IHttpRequest) => {
  const INPUT_TOPIC = process.env.INPUT_TOPIC || "input";
  try {
    await cache.start();
    await producer.start();
  
    consumer.registerHandlers(new Map<topic, IEventHandler>([
      [INPUT_TOPIC, async (message) => {
        await new GetClientAccessDataUseCase(producer, apiGeolocationRequester, cache).execute(message)
      }]
    ]));
