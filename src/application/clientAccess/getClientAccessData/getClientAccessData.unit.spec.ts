...
describe("Get client data use case unit test", () => {
  ...
  it("Should get client data from API and send to stream if value is not in cache", async () => {
    ...
    const getClientAccessDataUseCase = new GetClientAccessDataUseCase(localMockStreamProducer, localMockHttpRequest, localMockCache);
    const cacheKey: string = `${cachedInput.clientID}-${cachedInput.ip}`

    await expect(getClientAccessDataUseCase.execute(cachedInput)).resolves.toEqual(undefined);
    ...
    expect(localMockHttpRequest.get).toHaveBeenCalledWith(`/${cachedInput.ip}`, {
      fields: [
        "latitude",
        "longitude",
        "country_name",
    ...
    expect(localMockCache.set).toHaveBeenCalledWith(cacheKey, JSON.stringify(expectedOutput), 30);
    expect(localMockStreamProducer.send).toHaveBeenCalled();
    expect(localMockStreamProducer.send).toHaveBeenCalledWith(outputTopic, { value: expectedOutput });
  ...
  it("Should get client data from API and send to stream if cache expired", async () => {
    ...
    localMockCache.set(cachedInput.ip, JSON.stringify(cachedExpectedOutput), 30);
    jest.runOnlyPendingTimers();
    await expect(getClientAccessDataUseCase.execute(cachedInput)).resolves.toEqual(undefined);
    expect(localMockCache.get).toHaveBeenCalled();
    expect(localMockCache.get).toHaveBeenCalledWith(cacheKey);
    ...
    expect(localMockCache.set).toHaveBeenCalled();
    expect(localMockCache.set).toHaveBeenCalledWith(cacheKey, JSON.stringify(cachedExpectedOutput), 30);
    expect(localMockStreamProducer.send).toHaveBeenCalled();
    expect(localMockStreamProducer.send).toHaveBeenCalledWith(outputTopic, { value: cachedExpectedOutput });
    localMockCache.delete(cacheKey);
  });
  ...
  it("Should throw an exception if API fails", async () => {
    ...
    const localMockCache = new MockCache();
    const getClientAccessDataUseCase = new GetClientAccessDataUseCase(localMockStreamProducer, localMockHttpRequest, localMockCache);
    const getSpy = jest.spyOn(localMockHttpRequest, "get").mockRejectedValue(new Error("Some http error"));

    await expect(getClientAccessDataUseCase.execute(input)).rejects.toThrow("Error executing GetClientDataUseCase");
  ...
  it("Should throw an exception if producer fails", async () => {
    ...
    const localMockCache = new MockCache();
    const getClientAccessDataUseCase = new GetClientAccessDataUseCase(localMockStreamProducer, localMockHttpRequest, localMockCache);
    const sendSpy = jest.spyOn(localMockStreamProducer, "send").mockRejectedValue(new Error("Some producer error"));
    ...
    expect(sendSpy).toHaveBeenCalledWith(outputTopic, { value: expectedOutput });
  });
})